...
AI_CONCURRENCY_OPTIONS = (1, 2, 3, 4, 5, 6, 7, 8)
...
class WizardState:
    ...
    ai_question_refs_by_source: dict[str, list[dict[str, str]]] = field(default_factory=dict)
    ai_question_refs_version: int = 0
...
def build_import_flow_state(
    ...
    *,
    ...
    return WizardState(
        ...
        batch_source_files=list(cleaned_paths),
        batch_source_files_text="; ".join(raw_paths),
        ai_source_files=list(cleaned_paths),
        ai_source_files_text="; ".join(raw_paths),
        ai_source_file_items=built_items,
        ai_question_refs_by_source=dict(question_refs_by_source or {}),
        ai_question_refs_version=max(0, int(question_refs_version)),
        ai_import_level_path=str(import_level_path or "").strip(),
