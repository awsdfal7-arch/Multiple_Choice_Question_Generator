...
from sj_generator.config import QwenConfig, load_qwen_config, save_qwen_config, to_qwen_llm_config
...
class QwenConfigDialog(QDialog):
    ...
    def _collect_cfg(self) -> QwenConfig | None:
        try:
    ...
    def _on_test_api(self) -> None:
        ...
        if cfg is None:
            return
        try:
            client = LlmClient(to_qwen_llm_config(cfg))
        ...
        QMessageBox.information(self, "测试通过", "千问 API 可用性测试通过。")

    def _cfg_key(self, cfg: QwenConfig) -> str:
