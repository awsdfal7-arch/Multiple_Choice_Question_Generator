...
from sj_generator.config import KimiConfig, load_kimi_config, save_kimi_config, to_kimi_llm_config


class KimiConfigDialog(QDialog):
    def __init__(self, parent=None) -> None:
        super().__init__(parent)
        self.setWindowTitle("Kimi 配置")
        ...
        self._api_key_edit.setPlaceholderText(
            "已从环境变量 KIMI_API_KEY 读取" if self._env_api_key else "未设置环境变量 KIMI_API_KEY"
        )
        self._api_key_hint = QLineEdit("API Key 仅从环境变量 KIMI_API_KEY 读取")
    ...
    def _collect_cfg(self) -> KimiConfig | None:
        try:
    ...
    def _on_test_api(self) -> None:
        ...
        if cfg is None:
            return
        try:
            client = LlmClient(to_kimi_llm_config(cfg))
        ...
        QMessageBox.information(self, "测试通过", "Kimi API 可用性测试通过。")

    def _cfg_key(self, cfg: KimiConfig) -> str:
